...
public:
	...
	static void ShowLoadLayout(bool* p_open) {
		...
		if (ImGui::Begin("Loading", p_open, ImGuiWindowFlags_NoCollapse)) {
			...
			if (animate_gone) {
				...
				if (animate) {
					progress += progress_dir * 0.8f * ImGui::GetIO().DeltaTime;
	...
	static bool IsValidCardNumber(const char* cardnumber) {
		...
		if (cardnumber[0] == '2' || cardnumber[0] == '4' || cardnumber[0] == '5') {
			// Remove spaces and check if the length is 16
			int length = 0;
			...
			return length == 16; // Valid card number length
		}
		return false; // Invalid card prefix
	}
			...
			sqlite3_finalize(selectStatement);
				if (!existingCards.empty()) {
					...
					for (const auto& cardInfo : existingCards) {
						...
						ImGui::SameLine();
						ImGui::BeginChild(Childname.c_str(), ImVec2(350.0f, 200.0f));
						if(ImGui::Button(Childname.c_str(), ImVec2(20.0f,20.f))){
							ImGui::Text("sfgsdfgsdf");
						}
						ImGui::EndChild();
						if (countcards % 2 == 0) {
							ImGui::Spacing();
	...
	static void ShowCheck(bool* p_open) {
		...
		for (size_t i = 0; i < orderedFoods.size(); i++) {
			...
			const double orderedcost = orderedCost[i];
			...
			if (it != foodCounts.end()) {
				...
				if (typeIt != foodTypes.end()) {
					...
					if (orderedFood != "") {
						...
						ImGui::Text("= %g$", multiplication); ImGui::NextColumn();
						ImGui::Separator();
					}
				}
			}
